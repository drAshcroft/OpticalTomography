...
namespace ImageViewer3D.Filters.Files
{
    public class CombineMIPTool : aEffectForm3D
    {
        ...
        void button2_Click(object sender, EventArgs e)
        {
            ...
            if (ret == DialogResult.OK)
            {
                ...
                for (int i = 0; i < 360; i++)
                {
                    ...
                    Bitmap holdingRed = ImagingTools.MakeBitmap(Projection);//,MinContrast,MaxContrast );

                    DoMIPProjection_OneSlice(ref Projection2, (double)i / 180d * Math.PI, OtherImage);
                    Bitmap holdingGreen = ImagingTools.MakeBitmap(Projection2);//,MinContrast,MaxContrast );
        ...
        double[,] Projection2 = new double[10, 10];
        ...
        protected override ImageViewer3D.DataHolder doEffect(ImageViewer3D.DataEnvironment3D dataEnvironment, ImageViewer3D.DataHolder SourceImage, ImageViewer.Filters.ReplaceStringDictionary PassData, params object[] Parameters)
        {
            ...
            if (mParameters == null || mParameters[0] == null || (string)mParameters[0] == "DefaultFile.raw")
            {
        ...
        public static  void DoMIPProjection_OneSlice(ref double[,] SliceProjection, double AngleRadians, DataHolder SourceImage)
        {
        ...
        public static  double[,] ProjectMIP(double[, ,] mDataDouble, Point3D Direction, Point3D FastScanDirection)
        {
