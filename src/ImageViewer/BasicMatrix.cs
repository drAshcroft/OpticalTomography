...
namespace ImageViewer
{
    public static class BasicMatrix
    {
        public static float[,] Multiply(float[,] Matrix1, float[,] Matrix2)
        {
            ...
            {
                float[,] c =new float [Matrix1.GetLength(1), Matrix2.GetLength(0)];
                for (int i = 0; i < c.GetLength(0); i++)
                {
                    for (int j = 0; j < c.GetLength(1); j++)
                    {
                        ...
                        for (int k = 0; k < Matrix1.GetLength(0); k++) 
                            c[i, j] = c[i, j] + Matrix1[k,i] * Matrix2[j,k];
                    }
                }
                return c;
            }
        ...
        public static double[,] Multiply(double[,] Matrix1, double[,] Matrix2)
        {
            if (Matrix1.GetLength(1) == Matrix2.GetLength(0))
            {
                double[,] c = new double[Matrix1.GetLength(1), Matrix2.GetLength(0)];
                for (int i = 0; i < c.GetLength(0); i++)
                {
                    for (int j = 0; j < c.GetLength(1); j++)
                    {
        ...
        {
            double[,] a = new double[matrix.GetLength(1), matrix.GetLength(0)];
            for (int i = 0; i < matrix.GetLength(0); i++)
                for (int j = 0; j < matrix.GetLength(1); j++)
                    a[j, i] = matrix[i, j];
            return a;
