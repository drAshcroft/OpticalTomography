...
namespace ImageViewer.PythonScripting
{
    public partial class MacroRecorder : Form
    {
        ...
        protected ImageHolder mLastProcessed;
        protected ReplaceStringDictionary mPassData;
        #endregion

        #region effect stuff

        protected object doEffect(ImageViewer.DataEnvironment dataEnvironment, object SourceImage, ReplaceStringDictionary PassData, params object[] Parameters)
        {
        ...
        private void doSetup(DataEnvironment dataEnvironment, object[] MacroToken)
        {
            mDataEnvironment = dataEnvironment;
            ...
            */
            try
            {
                myViewer = dataEnvironment.Screen.ViewerControl;
                EventListener = new MacroLineGeneratedEvent(MacroLineGenerated);
                dataEnvironment.Screen.RegisterMacroListener(EventListener);
            }
            catch { }
        }
...
PostBatchProcess(dataEnvironment, ImageFileListIn, OutFileList, ScriptParams)";
        ...
        {
            AcceptInput = false;
            mDataEnvironment.Screen.RemoveMacroListener(EventListener);
        }
...
"Please choose a directory and filename for the output.  Files will be outputed with an added index, i.e.  If you give c:\\test.bmp, the program will output c;\\test0.bmp, c:\\test1.bmp ...");
            ...
            if (ret == DialogResult.OK)
            {
                ...
                ImageDisplayer id = new ImageDisplayer(mDataEnvironment);
                ...
                string[] Sorted =  MathStringHelps. SortNumberedFiles(filenames);
