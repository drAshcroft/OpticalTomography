    ...
    public static class MathFFTHelps
    {
        ...
        static MathFFTHelps()
        {
            try
            {
                ...
                Assembly fftwAsm = Assembly.LoadFrom(Path.GetDirectoryName(Application.ExecutablePath) + "\\fftwlib.dll");
        ...
        public static double[,] PowerSpectrum(List<double[,]> ROIs)
        {
            ...
            for (int i = 0; i < ROIs.Count; i++)
            {
                ...
                double[] XWindow = Filtering.HanWindow(aveg.GetLength(0), aveg.GetLength(0));
                double[] YWindow = Filtering.HanWindow(aveg.GetLength(1), aveg.GetLength(1));
