...
namespace Dehydrate
{
    class DehydrateScript:Tomographic_Imaging_2.IScript 
    {
        ...
        private ImageHolder ProcessImageClip(DataEnvironment dataEnvironment, int ImageNumber, ImageHolder BitmapImage)
        {
            ...
            try
            {
                Directory.CreateDirectory(dataEnvironment.ExperimentFolder + "Dehydrated\\");
            }
            catch { }
            BitmapImage.Save(dataEnvironment.ExperimentFolder + "Dehydrated\\" + string.Format("{0:000}", ImageNumber) + ".png");
        ...
        private void PreBatchProcessFindAndClip(DataEnvironment dataEnvironment, List<ImageFile> ImageFileList, Dictionary<string, string> ParamDictionary)
        {
            ...
            CellSize = (int)((int)PassData["MaxCellSize"] * 1.5);
            CellHalf = CellSize / 2;
