...
{
    public delegate void RefreshDataStoreEvent(out double[, ,] MyData, out double[,] Slice,out double MinContrastPercent,out double MaxContrastPercent);
    public delegate void UpdateContrastEvent(double MinContrast, double MaxContrast);
    public partial class LUTTool : aEffectForm
    {
        ...
        void trackBar2_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (UpdateContrast != null)
                UpdateContrast((double)pars[0], (double)pars[1]);
            mFilterToken = CreateToken(pars);
            DoRun();
        }
        ...
        public event UpdateContrastEvent UpdateContrast;

        public override string EffectName { get { return "LUT 3D Adjustment"; } }
        ...
        {
            Bitmap holding = MakeBitmap(mSlice, (double)FilterToken.Parameters[1], (double)FilterToken.Parameters[0]);
            return holding;
        }
        ...
        public override string RunEffect(ScreenProperties[] SourceImage, IEffectToken ContrastToken)
        {
            if (ContrastToken == null)
            {
                ...
                ContrastToken.Parameters[0] = .25;
                ContrastToken.Parameters[1] = .75;
            }
            ...
            ContrastToken.Parameters[0] = minContrast;
            ContrastToken.Parameters[1] = maxContrast;

            mSourceImages = SourceImage;

            double[] Flattened = ClipImage(mData);
            ...
            mFilterToken.Parameters[0] = minContrast  * (MaxValue - MinValue) + MinValue;
            mFilterToken.Parameters[0] = maxContrast  * (MaxValue - MinValue) + MinValue;
            mScratchImage = mSlice.MakeBitmap();

            pictureBox1.Image = RunEffect(mScratchImage, mFilterToken);
        ...
        {
            Preview.Image = RunEffect(mScratchImage, mFilterToken);
            Preview.Invalidate();
        }
    }
}
