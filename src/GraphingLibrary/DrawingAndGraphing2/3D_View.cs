...
namespace GraphingLibrary.DrawingAndGraphing2
{
    public partial class _3D_View : UserControl
    {
        ...
        /// only for showing data
        /// </summary>
        /// <param name="DirectoryPath">Directory where a stack of images is sequencial numbered</param>
        public void IsoSurfaceImages(string DirectoryPath)
        {
            string[] files = Directory.GetFiles(DirectoryPath);
            files = MathHelpLib.MathStringHelps.SortNumberedFiles(files);

            Bitmap b = new Bitmap(files[0]);
            ushort[, ,] DataCube = new ushort[b.Width / 4 + 2, b.Height / 4 + 2, files.Length / 4 + 2];
            ...
            for (int i = 0; i < files.Length; i++)
            {
                ...
                double sum = imageData.SumArray();
                double max = imageData.MaxArray();
                b.Dispose();

                for (int x = 0; x < imageData.GetLength(0); x++)
                    for (int y = 0; y < imageData.GetLength(1); y++)
                        DataCube[
                            (int)((double)x / 4) % DataCube.GetLength(0) + 1,
                            (int)((double)y / 4) % DataCube.GetLength(1) + 1,
                            (int)((double)i / 4) % DataCube.GetLength(2) + 1] = (ushort)imageData[x, y];
            }
