...
/*
int SimpleConvolution::genBinaryImage()
{
...
int SimpleConvolution::setupCL(void)
{
    cl_int status = 0;
    ...
    maskBuffer = clCreateBuffer(
            context, 
            CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR,
            sizeof(cl_double ) * maskWidth * maskHeight,
            mask, 
            &status);
    if(!sampleCommon->checkVal(
                status,
    ...
    kernel = clCreateKernel(program , KernalName.c_str() /*"simpleConvolution"*/, &status);
    if(!sampleCommon->checkVal(
                status,
                CL_SUCCESS,
...
int SimpleConvolution::resetInput()
{
	cl_int status = 0;
...
int SimpleConvolution::resetInputOutput()
{
	cl_int status = 0;
...
/*
extern "C" __declspec( dllexport ) int CreateConvoluter(float* OutputImage, float* Image, int ImageWidth,int ImageHeight, float* Filter, int FilterWidth,int FilterHeight)
{
	clSimpleConvolution=new  SimpleConvolution("OpenCL Simple Convolution");
	
	clSimpleConvolution->height =ImageHeight;
	clSimpleConvolution->width =ImageWidth;
	clSimpleConvolution->input =Image;

	clSimpleConvolution->output = OutputImage;

	clSimpleConvolution->mask=Filter;
	clSimpleConvolution->maskWidth=FilterWidth;
	clSimpleConvolution->maskHeight =FilterHeight;
...
*/
extern "C" __declspec( dllexport ) SimpleConvolution* CreateConvoluter(char* KernalName, char* ConvolutionCode, char* FlagStr,
	double* OutputImage, double* Image, int ImageWidth,int ImageHeight, double* Filter, int FilterWidth,int FilterHeight)
{
	...
	clSimpleConvolution->ConvolutionCode=std::string(ConvolutionCode);
	clSimpleConvolution->flagsStr = std::string(FlagStr);
	clSimpleConvolution->KernalName = std::string(KernalName);
	...
	clSimpleConvolution->maskHeight =FilterHeight;
	 ...
	 if(clSimpleConvolution->setup()!=SDK_SUCCESS)
	 {
		...
		clSimpleConvolution=NULL;
	 }

	return clSimpleConvolution;
}


extern "C" __declspec( dllexport ) int RunConvoluter(SimpleConvolution* clSimpleConvolution,double* Image)
{
	clSimpleConvolution->input =Image;
	clSimpleConvolution->resetInput();
...
extern "C" __declspec( dllexport ) int RunConvoluterInputOutput(SimpleConvolution* clSimpleConvolution, double* Image, double* outputImage)
{
	clSimpleConvolution->input =Image;
	clSimpleConvolution->output = outputImage;
	clSimpleConvolution->resetInputOutput();
...
extern "C" __declspec( dllexport ) int CloseConvoluter(SimpleConvolution* clSimpleConvolution)
{
	 if(clSimpleConvolution->cleanup()!=SDK_SUCCESS)
          return SDK_FAILURE;
