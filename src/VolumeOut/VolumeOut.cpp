...
#include "VolumeOut.hpp"


int VolumeOut::setupFBP_Projection()
{
...
int VolumeOut::setupCL(void)
{
	...
	*/
	...
	if(!sampleCommon->checkVal(	status,	CL_SUCCESS,	"clCreateSampler failed."))
		return SDK_FAILURE;
	...
	inputBuffer = clCreateBuffer( context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, sizeof(cl_double ) * width * height, input, &status);
...
int VolumeOut::runCLKernels(void)
{
	cl_int   status;
	cl_event events[2];
...
{
	VolumeOut clFBP_Projection("OpenCL Simple Convolution");
...
extern "C" __declspec( dllexport ) VolumeOut* CreateVolumeOut(char* KernalName, char* ConvolutionCode, char* FlagStr, 
	double* Image, int ImageWidth,int ImageHeight, double* Cube, int CubeWidth,int CubeHeight, int CubeDepth)
{
	...
	VolumeOut* clVolumeOut;

	clVolumeOut=new  VolumeOut("OpenCL Simple Convolution");

	clVolumeOut->ConvolutionCode=std::string(ConvolutionCode);
	clVolumeOut->flagsStr = std::string(FlagStr);
	clVolumeOut->KernalName = std::string(KernalName);



	std::cout <<clVolumeOut->KernalName;

	clVolumeOut->height =ImageHeight;
	clVolumeOut->width =ImageWidth;
	clVolumeOut->input =Image;

	clVolumeOut->Cube=Cube;
	clVolumeOut->CubeWidth=CubeWidth;
	clVolumeOut->CubeHeight =CubeHeight;
	clVolumeOut->CubeDepth =CubeDepth;


	if (CubeWidth>250)
	{
		clVolumeOut->SetDeviceType("cpu");
	}
	else
	{
		clVolumeOut->SetDeviceType("gpu");
	}

	std::cout << "Running Setup";

	if(clVolumeOut->setup()!=SDK_SUCCESS)
	{
...
extern "C" __declspec( dllexport ) int RunVolumeOut(VolumeOut* clVolumeOut,double* Image, float uX, float uY, float uZ, float aX, float aY, float aZ)
{
	...
	clVolumeOut->UP=UP;
	clVolumeOut->Across=Across;

	clVolumeOut->resetInput();

	if(clVolumeOut->run()!=SDK_SUCCESS)
		return SDK_FAILURE;
...
extern "C" __declspec( dllexport ) int getVolumeOutData(VolumeOut* clVolumeOut)
{

	if(clVolumeOut->getCLData()!=SDK_SUCCESS)
		return SDK_FAILURE;
...
extern "C" __declspec( dllexport ) int CloseVolumeOut(VolumeOut* clVolumeOut)
{
	if(clVolumeOut->cleanup()!=SDK_SUCCESS)
		return SDK_FAILURE;

	clVolumeOut->printStats();
	return SDK_SUCCESS;
}