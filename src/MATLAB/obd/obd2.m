function [x, f] = obd2(x, y, sf, maxiter, clipping, srf,f,origPSF,regularize)
...
%     end
    ...
    f = obd_updatePSF(f, x, y, maxiter(1), clipping, srf,regularize);
    sumf = sum(f(:));
    ...
    %   f=obd_updatePSF(origPSF, x, y, 15, clipping, srf);
    f=origPSF(:,:);
    sumf = sum(f(:));
    f = f/sumf;
    %f = obd_updatePSF(f, x, y, maxiter(1), clipping, srf,regularize);
...
%%%%%%%%%%%%%%%%
function f = obd_updatePSF(f, x, y, maxiter, clipping, srf ,regularize)
...
if regularize
    f(size(f,1)/2,size(f,2)/2)=1.5*f(size(f,1)/2,size(f,2)/2);
