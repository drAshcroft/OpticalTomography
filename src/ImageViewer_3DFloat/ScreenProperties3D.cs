...
namespace ImageViewer3D
{
    public class ScreenProperties3D
    {
        ...
        public bool Zooming
        {
            ...
            set { mDataEnvironment.Zooming = true; }
        }

        private double mZoomX, mZoomY,mZoom0X,mZoom0Y;
        ...
        /*  if (mDataEnvironment != null)
            {
                try
                {
                    ...
                    {
                        // ScreenBackBuffer.Dispose();
                        ScreenBackBuffer = new Bitmap(mPictureBox.Width, mPictureBox.Height, PixelFormat.Format32bppRgb);
                    }
                    if (ScreenFrontBuffer == null)
                    {
                        // ScreenFrontBuffer.Dispose();
                        ScreenFrontBuffer = new Bitmap(mPictureBox.Width, mPictureBox.Height, PixelFormat.Format32bppRgb);
                    }
        ...
        {
            if (mDataEnvironment != null && mPictureBox!=null)
            {
                Rectangle ViewBox = GetViewRectangle();
                Bitmap OriginalImage = mDataEnvironment.GetSlice(mAxis, mSlice);

                if (ScreenBackBuffer == null || ScreenFrontBuffer == null)
                {
                    ScreenFrontBuffer = new Bitmap(mPictureBox.Width, mPictureBox.Height, PixelFormat.Format32bppRgb);
                }
                ...
                if (mDataEnvironment.Zooming == true)
                {
                    ...
                    g2.Clear(Color.Black);
                    g2.InterpolationMode = mDataEnvironment.InterpolationMode;
                    g2.DrawImage(OriginalImage, new Rectangle(0, 0, mPictureBox.Width , mPictureBox.Height ), ViewBox, GraphicsUnit.Pixel);
                    mZoom0X = ViewBox.X;
                    mZoom0Y = ViewBox.Y;
                    mZoomX = (double)mPictureBox.Width / (double)ViewBox.Width;
                    mZoomY = (double)mPictureBox.Height / (double)ViewBox.Height;
                    ScreenBackBuffer.Dispose();
                    ScreenBackBuffer = new Bitmap(ScreenFrontBuffer);
                }
                else
